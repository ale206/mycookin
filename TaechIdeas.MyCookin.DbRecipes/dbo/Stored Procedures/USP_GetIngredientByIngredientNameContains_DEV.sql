...
-- =============================================
CREATE PROCEDURE [dbo].[USP_GetIngredientByIngredientNameContains_DEV](@IngredientName nvarchar(100), @IDLanguage INT)
AS
BEGIN
...
SET @FulltextVar = REPLACE(REPLACE(@IngredientName,'"',' '),'''',' ') 	
	...
	BEGIN TRY
		...
		SET @Sql = 'SELECT IDIngredientLanguage, IDIngredient, IngredientSingular,Lenght, MIN(Relevance) AS Relevance  FROM		
					(
					...
					WHERE IngredientSingular='''+@FulltextVar+''' OR IngredientPlural='''+@FulltextVar+'''
					UNION
	...
	BEGIN CATCH
		...
		SELECT  TOP 15 IDIngredientLanguage, Ingredients.IDIngredient, IngredientSingular
