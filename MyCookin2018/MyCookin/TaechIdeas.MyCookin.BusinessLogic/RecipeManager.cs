...
namespace TaechIdeas.MyCookin.BusinessLogic
{
    public class RecipeManager : IRecipeManager
    {
        ...
        public RecipeManager(IRecipeConfig recipeConfig, IRecipeRepository recipeRepository, IUtilsManager utilsManager,
            IMediaManager mediaManager, IUserManager userManager, INetworkConfig networkConfig,
            ITokenManager tokenManager, IIngredientManager ingredientManager,
        ...
        {
            if (searchRecipesInput == null) throw new ArgumentException("SearchRecipesInput Object Is Null");
            ...
            else
            {
                ...
                searchRecipesOutput = searchRecipesInput.IsEmptyFridge
                    ? SearchRecipesWithEmptyFridge(searchRecipesInput)
                    : SearchRecipesWithoutEmptyFridge(searchRecipesInput);
            }
        ...
        public string AddPropertiesJoined(int idLanguage, bool isVegan, bool isVegetarian, bool isGlutenFree)
        {
            ...
            {
                case 1:
                    if (isVegan) propertiesJoined.Append("Vegan, ");

                    if (isVegetarian) propertiesJoined.Append("Vegetarian, ");

                    if (isGlutenFree) propertiesJoined.Append("Gluten Free");

                    break;
                case 2:
                    if (isVegan) propertiesJoined.Append("Vegana, ");

                    if (isVegetarian) propertiesJoined.Append("Vegetariana, ");

                    if (isGlutenFree) propertiesJoined.Append("Senza Glutine");
                ...
                case 3:
                    ...
                    if (isGlutenFree) propertiesJoined.Append("Sin Gluten");
        ...
        public IEnumerable<TopRecipesByLanguageOutput> TopRecipesByLanguage(
            TopRecipesByLanguageInput topRecipesByLanguageInput)
        {
            ...
            var topRecipeResultList =
                _mapper.Map<IEnumerable<TopRecipesByLanguageOutput>>(
                    _recipeRepository.TopRecipesByLanguage(
                        _mapper.Map<TopRecipesByLanguageIn>(topRecipesByLanguageInput)));

            var topRecipesByLanguageOutputs = topRecipeResultList as IList<TopRecipesByLanguageOutput> ??
                                              topRecipeResultList.ToList();

            //Add FriendlyId (For path url) //TODO: GET FROM QUERY
            topRecipesByLanguageOutputs.ToList()
                .ForEach(c => c.FriendlyId = FriendlyIdByRecipeLanguageId(new FriendlyIdByRecipeLanguageIdInput
                    {RecipeLanguageId = c.RecipeLanguageId}).FriendlyId);
        ...
        public IEnumerable<BestRecipesByLanguageOutput> BestRecipesByLanguage(
            BestRecipesByLanguageInput bestRecipesByLanguageInput)
        {
            var bestRecipesByLanguageOut =
                _recipeRepository.BestRecipesByLanguage(
                    _mapper.Map<BestRecipesByLanguageIn>(bestRecipesByLanguageInput));

            var bestRecipeResultList =
                _mapper.Map<IEnumerable<BestRecipesByLanguageOutput>>(bestRecipesByLanguageOut);

            var bestRecipesByLanguageOutputs = bestRecipeResultList as IList<BestRecipesByLanguageOutput> ??
                                               bestRecipeResultList.ToList();
        ...
        public string GetGoogleSuggestions(string words)
        {
            try
            {
                if (!_recipeConfig.UseGoogleSuggestionsForSearchRecipes) return string.Empty;
        ...
        {
            var recipeByIdOutput =
                _mapper.Map<RecipeByIdOutput>(_recipeRepository.RecipeById(_mapper.Map<RecipeByIdIn>(recipeByIdInput)));
        ...
        {
            var recipeByFriendlyIdOutput = _mapper.Map<RecipeByFriendlyIdOutput>(
                _recipeRepository.RecipeByFriendlyId(_mapper.Map<RecipeByFriendlyIdIn>(recipeByFriendlyIdInput)));
            ...
            var recipeByIdAndLanguageOutput =
                RecipeByIdAndLanguage(new RecipeByIdAndLanguageInput
                    {LanguageId = recipeByFriendlyIdOutput.LanguageId, RecipeId = recipeByFriendlyIdOutput.RecipeId});
            ...
            recipeByFriendlyIdOutput.PropertiesJoined = AddPropertiesJoined(recipeByIdAndLanguageOutput.LanguageId,
                recipeByIdAndLanguageOutput.IsVegan, recipeByIdAndLanguageOutput.IsVegetarian,
            ...
            recipeByFriendlyIdOutput.ImageUrl = recipeByFriendlyIdOutput.RecipeImageId == null
                ...
                : _mediaManager.MediaPathByMediaId(new MediaPathByMediaIdInput
                    {MediaId = (Guid) recipeByFriendlyIdOutput.RecipeImageId}).MediaPath;
            ...
            else
            {
                ...
                recipeByFriendlyIdOutput.RecipeOwner =
                    _mapper.Map<RecipeOwnerOutput>(_userManager.UserById(userByIdInput, true));
            }
            ...
            {
                recipeByFriendlyIdOutput.Steps = StepsForRecipe(new StepsForRecipeInput
                    {RecipeLanguageId = recipeByFriendlyIdOutput.RecipeLanguageId});

                var stepForRecipeOutputs = recipeByFriendlyIdOutput.Steps as IList<StepForRecipeOutput> ??
                                           recipeByFriendlyIdOutput.Steps.ToList();
                if (stepForRecipeOutputs.Any())
                    //Add Steps ImageUrl
                    stepForRecipeOutputs.ToList()
                        .ForEach(c => c.ImageUrl = c.RecipeStepImageId == null
                            ? null
                            : _mediaManager.MediaPathByMediaId(new MediaPathByMediaIdInput
                                {MediaId = (Guid) c.RecipeStepImageId}).MediaPath);
            }

            //Add Ingredients
            if (recipeByFriendlyIdInput.IncludeIngredients)
            {
                var allIngredients = _ingredientManager.IngredientsByIdRecipeAndLanguage(
                    new IngredientsByIdRecipeAndLanguageInput
                    {
                ...
                var ingredientByIdRecipeAndLanguageOutputs =
                    allIngredients as IList<IngredientsByIdRecipeAndLanguageOutput> ?? allIngredients.ToList();

                recipeByFriendlyIdOutput.IngredientsForRecipes = ingredientByIdRecipeAndLanguageOutputs.Where(x =>
                    x.RecipeIngredientGroupNumber == (int) IngredientGroupNumber.Recipe);
                recipeByFriendlyIdOutput.IngredientsForDough = ingredientByIdRecipeAndLanguageOutputs.Where(x =>
                    x.RecipeIngredientGroupNumber == (int) IngredientGroupNumber.Dough);
                recipeByFriendlyIdOutput.IngredientsForDressing = ingredientByIdRecipeAndLanguageOutputs.Where(x =>
                    x.RecipeIngredientGroupNumber == (int) IngredientGroupNumber.Dressing);
                recipeByFriendlyIdOutput.IngredientsForFilling = ingredientByIdRecipeAndLanguageOutputs.Where(x =>
                    x.RecipeIngredientGroupNumber == (int) IngredientGroupNumber.Filling);
                recipeByFriendlyIdOutput.IngredientsForSauce = ingredientByIdRecipeAndLanguageOutputs.Where(x =>
                    x.RecipeIngredientGroupNumber == (int) IngredientGroupNumber.Sauce);
                recipeByFriendlyIdOutput.IngredientsForDecoration = ingredientByIdRecipeAndLanguageOutputs.Where(x =>
                    x.RecipeIngredientGroupNumber == (int) IngredientGroupNumber.Decoration);
            }
        ...
        {
            return _mapper.Map<GenerateFriendlyIdOutput>(
                _recipeRepository.GenerateFriendlyId(_mapper.Map<GenerateFriendlyIdIn>(generateFriendlyIdInput)));
        }
        ...
        public FriendlyIdByRecipeLanguageIdOutput FriendlyIdByRecipeLanguageId(
            FriendlyIdByRecipeLanguageIdInput friendlyIdByRecipeLanguageIdInput)
        {
            return _mapper.Map<FriendlyIdByRecipeLanguageIdOutput>(
                _recipeRepository.FriendlyIdByRecipeLanguageId(
                    _mapper.Map<FriendlyIdByRecipeLanguageIdIn>(friendlyIdByRecipeLanguageIdInput)));
        }
        ...
        public UpdateRecipeLanguageOutput UpdateRecipeLanguage(UpdateRecipeLanguageInput updateRecipeLanguageInput)
        {
            ...
            var updateRecipeLanguageOutput =
                _mapper.Map<UpdateRecipeLanguageOutput>(_recipeRepository.UpdateRecipeLanguage(updateRecipeLanguageIn));
            if (!updateRecipeLanguageOutput.RecipeLanguageUpdated)
                throw new Exception("Error on UpdateRecipeLanguageOutput");
        ...
        public UpdateRecipeLanguageStepOutput UpdateRecipeLanguageStep(
            UpdateRecipeLanguageStepInput updateRecipeLanguageStepInput)
        {
            //Check for Valid Token
            ...
            var updateRecipeLanguageStepsOutput =
                _mapper.Map<UpdateRecipeLanguageStepOutput>(
                    _recipeRepository.UpdateRecipeLanguageStep(
                        _mapper.Map<UpdateRecipeLanguageStepIn>(updateRecipeLanguageStepInput)));
            if (!updateRecipeLanguageStepsOutput.RecipeLanguageStepsUpdated)
                throw new Exception("Error on UpdateRecipeLanguageStep");
        ...
        public IEnumerable<NewRecipeLanguageStepsOutput> NewRecipeLanguageSteps(
            IEnumerable<NewRecipeLanguageStepsInput> newRecipeLanguageStepsInput)
        {
            //Check for Valid Token
            var recipeLanguageStepsInputs = newRecipeLanguageStepsInput as IList<NewRecipeLanguageStepsInput> ??
                                            newRecipeLanguageStepsInput.ToList();
            ...
            var newRecipeLanguageStepsInputs = newRecipeLanguageStepsInput as IList<NewRecipeLanguageStepsInput> ??
                                               recipeLanguageStepsInputs.ToList();

            foreach (var newRecipeLanguageStepInput in newRecipeLanguageStepsInputs)
            {
                //Update RecipeLanguage
                var newRecipeLanguageStepsOutput =
                    _mapper.Map<NewRecipeLanguageStepsOutput>(
                        _recipeRepository.NewRecipeLanguageStep(
                            _mapper.Map<NewRecipeLanguageStepIn>(newRecipeLanguageStepInput)));
                if (!newRecipeLanguageStepsOutput.NewRecipeLanguageStepInserted)
                    throw new Exception("Error on NewRecipeLanguageStep");
        ...
        public UpdateRecipeIngredientOutput UpdateRecipeIngredient(
            UpdateRecipeIngredientInput updateRecipeIngredientInput)
        {
            //Check for Valid Token
            ...
            //Update RecipeLanguage
            var updateRecipeIngredientOutput = _mapper.Map<UpdateRecipeIngredientOutput>(
                _recipeRepository.UpdateRecipeIngredient(
                    _mapper.Map<UpdateRecipeIngredientIn>(updateRecipeIngredientInput)));
            if (!updateRecipeIngredientOutput.RecipeIngredientUpdated)
                throw new Exception("Error on UpdateRecipeIngredient");

            //Update NutritionalFacts //TODO CHECK!!
            var calculateNutritionalFactsOutput =
                CalculateNutritionalFacts(_mapper.Map<CalculateNutritionalFactsInput>(updateRecipeIngredientInput));
        ...
        public IEnumerable<AddNewIngredientToRecipeOutput> AddNewIngredientToRecipe(
            IEnumerable<AddNewIngredientToRecipeInput> addNewIngredientToRecipeInput)
        {
            //Check for Valid Token
            var newIngredientToRecipeInputs = addNewIngredientToRecipeInput as IList<AddNewIngredientToRecipeInput> ??
                                              addNewIngredientToRecipeInput.ToList();
            ...
            var addNewIngredientToRecipeInputs =
                addNewIngredientToRecipeInput as IList<AddNewIngredientToRecipeInput> ??
                newIngredientToRecipeInputs.ToList();

            foreach (var newIngredientToRecipeInput in addNewIngredientToRecipeInputs)
            {
                //Update RecipeLanguage
                var updateRecipeLanguageOutput =
                    _mapper.Map<AddNewIngredientToRecipeOutput>(
                        _recipeRepository.AddNewIngredientToRecipe(
                            _mapper.Map<AddNewIngredientToRecipeIn>(newIngredientToRecipeInput)));
                if (!updateRecipeLanguageOutput.NewIngredientToRecipeAdded)
                    throw new Exception("Error on AddNewIngredientToRecipe");
            ...
            var calculateNutritionalFactsInput = new CalculateNutritionalFactsInput
                {RecipeId = addNewIngredientToRecipeInputs.First().RecipeId};
            var calculateNutritionalFactsOutput =
                CalculateNutritionalFacts(_mapper.Map<CalculateNutritionalFactsInput>(calculateNutritionalFactsInput));
        ...
        public IEnumerable<RecipeLanguageListOutput> RecipeLanguageList(RecipeLanguageListInput recipeLanguageListInput)
        {
            ...
            return _mapper.Map<IEnumerable<RecipeLanguageListOutput>>(
                _recipeRepository.RecipeLanguageList(_mapper.Map<RecipeLanguageListIn>(recipeLanguageListInput)));
        }
        ...
        public CalculateNutritionalFactsOutput CalculateNutritionalFacts(
            CalculateNutritionalFactsInput calculateNutritionalFactsInput)
        {
            return _mapper.Map<CalculateNutritionalFactsOutput>(
                _recipeRepository.CalculateNutritionalFacts(
                    _mapper.Map<CalculateNutritionalFactsIn>(calculateNutritionalFactsInput)));
        }
        ...
        public DeleteRecipeOutput DeleteRecipe(DeleteRecipeInput deleteRecipeInput)
        {
            ...
            var deleteRecipeLanguageOutput =
                _mapper.Map<DeleteRecipeOutput>(
                    _recipeRepository.DeleteRecipe(_mapper.Map<DeleteRecipeIn>(deleteRecipeInput)));
        ...
        public IEnumerable<AddOrUpdatePropertyValueOutput> AddOrUpdatePropertyValue(CheckTokenInput checkTokenInput,
            IEnumerable<AddOrUpdatePropertyValueInput> addOrUpdatePropertyValueInput)
        {
            //Check for Valid Token
            ...
            return
                addOrUpdatePropertyValueInput.Select(
                    propertyValueInput => _mapper.Map<AddOrUpdatePropertyValueOutput>(
                        _recipeRepository.AddOrUpdatePropertyValue(
                            _mapper.Map<AddOrUpdatePropertyValueIn>(propertyValueInput))));
        ...
        public DeletePropertyValueOutput DeletePropertyValue(DeletePropertyValueInput deletePropertyValueInput)
        {
            ...
            return _mapper.Map<DeletePropertyValueOutput>(
                _recipeRepository.DeletePropertyValue(_mapper.Map<DeletePropertyValueIn>(deletePropertyValueInput)));
        }
        ...
        public IEnumerable<RecipePropertyByIdRecipeAndLanguageOutput> RecipePropertyByIdRecipeAndLanguage(
            RecipePropertyByIdRecipeAndLanguageInput recipePropertyByIdRecipeAndLanguageInput)
        {
            return
                _mapper.Map<IEnumerable<RecipePropertyByIdRecipeAndLanguageOutput>>(
                    _recipeRepository.RecipePropertyByIdRecipeAndLanguage(
                        _mapper.Map<RecipePropertyByIdRecipeAndLanguageIn>(recipePropertyByIdRecipeAndLanguageInput)));
        }
        ...
        public IEnumerable<StepsByIdRecipeAndLanguageOutput> StepsByIdRecipeAndLanguage(
            StepsByIdRecipeAndLanguageInput stepsByIdRecipeAndLanguageInput)
        {
            var stepsByIdRecipeAndLanguageOutput =
                _mapper.Map<IEnumerable<StepsByIdRecipeAndLanguageOutput>>(
                    _recipeRepository.StepsByIdRecipeAndLanguage(
                        _mapper.Map<StepsByIdRecipeAndLanguageIn>(stepsByIdRecipeAndLanguageInput)));

            //Add Images
            var stepsByIdRecipeAndLanguageOutputs =
                stepsByIdRecipeAndLanguageOutput as IList<StepsByIdRecipeAndLanguageOutput> ??
                stepsByIdRecipeAndLanguageOutput.ToList();
        ...
        {
            return _mapper.Map<IEnumerable<StepForRecipeOutput>>(
                _recipeRepository.StepsForRecipe(_mapper.Map<StepsForRecipeIn>(stepsForRecipeInput)));
        }
        ...
        {
            return _mapper.Map<IEnumerable<RecipeSiteMapOutput>>(
                _recipeRepository.RecipeSiteMap(_mapper.Map<RecipeSiteMapIn>(recipeSiteMapInput)));
        }
        ...
        {
            return _mapper.Map<IEnumerable<RecipesByOwnerOutput>>(
                _recipeRepository.RecipesByOwner(_mapper.Map<RecipesByOwnerIn>(recipesByOwnerInput)));
        }
        ...
        public TranslateBunchOfRecipesOutput TranslateBunchOfRecipes(
            TranslateBunchOfRecipesInput translateBunchOfRecipesInput)
        {
            var recipesToTranslateOut =
                _recipeRepository.RecipesToTranslate(_mapper.Map<RecipesToTranslateIn>(translateBunchOfRecipesInput));

            var recipesToTranslateOuts =
                recipesToTranslateOut as IList<RecipesToTranslateOut> ?? recipesToTranslateOut.ToList();
            if (!recipesToTranslateOuts.Any()) return null;
            ...
            foreach (var recipeToTranslate in recipesToTranslateOuts)
            {
                ...
                addNewRecipeTranslationIn.RecipeName = string.IsNullOrEmpty(recipeToTranslate.RecipeName)
                    ...
                    : _microsoftTranslationManager
                        .TranslateSentence(languageFrom, languageTo, recipeToTranslate.RecipeName).TranslatedSentence;
                addNewRecipeTranslationIn.RecipeHistory = string.IsNullOrEmpty(recipeToTranslate.RecipeHistory)
                    ? null
                    : _microsoftTranslationManager
                        .TranslateSentence(languageFrom, languageTo, recipeToTranslate.RecipeHistory)
                        .TranslatedSentence;
                addNewRecipeTranslationIn.RecipeNote = string.IsNullOrEmpty(recipeToTranslate.RecipeNote)
                    ? null
                    : _microsoftTranslationManager
                        .TranslateSentence(languageFrom, languageTo, recipeToTranslate.RecipeNote).TranslatedSentence;
                addNewRecipeTranslationIn.RecipeSuggestion = string.IsNullOrEmpty(recipeToTranslate.RecipeSuggestion)
                    ? null
                    : _microsoftTranslationManager
                        .TranslateSentence(languageFrom, languageTo, recipeToTranslate.RecipeSuggestion)
                        .TranslatedSentence;
                addNewRecipeTranslationIn.RecipeLanguageTags =
                    string.IsNullOrEmpty(recipeToTranslate.RecipeLanguageTags)
                        ? null
                        : _microsoftTranslationManager
                            .TranslateSentence(languageFrom, languageTo, recipeToTranslate.RecipeLanguageTags)
                            .TranslatedSentence;
        ...
        {
            return _mapper.Map<IEnumerable<SimilarRecipesOutput>>(
                _recipeRepository.SimilarRecipes(_mapper.Map<SimilarRecipesIn>(similarRecipesInput)));
        }
        ...
        {
            return _mapper.Map<CalculateRecipeTagsOutput>(
                _recipeRepository.CalculateRecipeTags(_mapper.Map<CalculateRecipeTagsIn>(calculateRecipeTagsInput)));
        }
        ...
        {
            return _mapper.Map<IEnumerable<StepsToTranslateOutput>>(
                _recipeRepository.StepsToTranslate(_mapper.Map<StepsToTranslateIn>(stepsToTranslateInput)));
        }
        ...
        {
            return _mapper.Map<IEnumerable<RecipeLanguageTagsOutput>>(
                _recipeRepository.RecipeLanguageTags(_mapper.Map<RecipeLanguageTagsIn>(recipeLanguageTagsInput)));
        }
        ...
        public IEnumerable<TagsByRecipeAndLanguageOutput> TagsByRecipeAndLanguage(
            TagsByRecipeAndLanguageInput recipeAndLanguageInput)
        {
            return _mapper.Map<IEnumerable<TagsByRecipeAndLanguageOutput>>(
                _recipeRepository.TagsByRecipeAndLanguage(
                    _mapper.Map<TagsByRecipeAndLanguageIn>(recipeAndLanguageInput)));
        }
        ...
        public PropertiesByRecipeAndLanguageOutput PropertiesByRecipeAndLanguage(
            PropertiesByRecipeAndLanguageInput propertiesByRecipeAndLanguageInput)
        {
            ...
            var propertiesByRecipeAndLanguageOut =
                _recipeRepository.PropertiesByRecipeAndLanguage(
                    _mapper.Map<PropertiesByRecipeAndLanguageIn>(propertiesByRecipeAndLanguageInput));

            var propertiesByRecipeAndLanguageOuts =
                propertiesByRecipeAndLanguageOut as IList<PropertiesByRecipeAndLanguageOut> ??
                propertiesByRecipeAndLanguageOut.ToList();
            var recipePropertyTypeIds =
                propertiesByRecipeAndLanguageOuts.Select(x => x.IDRecipePropertyType).Distinct();

            //TODO: IMPROVE!!
            foreach (var recipePropertyTypeId in recipePropertyTypeIds)
            {
                var byRecipeAndLanguageOut =
                    propertiesByRecipeAndLanguageOuts.FirstOrDefault(x =>
                        x.IDRecipePropertyType.Equals(recipePropertyTypeId));
                if (byRecipeAndLanguageOut != null)
                    groupList.Add(
                        new PropertiesByRecipeAndLanguageGroupOutput
                        {
                            ...
                            RecipeProperties = propertiesByRecipeAndLanguageOuts
                                .Where(x => x.IDRecipePropertyType.Equals(recipePropertyTypeId))
                                .Select(x => x.RecipeProperty)
                        });
            }
        ...
        private IEnumerable<SearchRecipesOutput> SearchRecipesWithoutEmptyFridge(SearchRecipesInput searchRecipesInput)
        {
            ...
            IList<SearchRecipesOut> searchRecipesResultDto =
                _recipeRepository.SearchRecipes(_mapper.Map<SearchRecipesIn>(searchRecipesInput)).ToList();
            ...
            foreach (var searchRecipesOut in searchRecipesResultDto)
                searchRecipesOutputsList.Add(
                    _mapper.Map<SearchRecipesOutput>(RecipeByFriendlyId(new RecipeByFriendlyIdInput
                        {FriendlyId = searchRecipesOut.FriendlyId}))
                );
        ...
        private IEnumerable<SearchRecipesOutput> SearchRecipesWithEmptyFridge(SearchRecipesInput searchRecipesInput)
        {
            ...
            foreach (var ingrName in ingredientSplitted)
            {
                ...
                if (!string.IsNullOrEmpty(suggestedName)) completeIngredientList += suggestedName.Trim() + ',';
            }

            searchRecipesInput.Search = completeIngredientList.Substring(0, completeIngredientList.Length - 1);

            var searchEmptyFridgeRecipesResultDto = _recipeRepository
                .SearchRecipesWithEmptyFridge(_mapper.Map<SearchRecipesWithEmptyFridgeIn>(searchRecipesInput)).ToList();
        ...
        {
            if (recipeByIdAndLanguageInput == null) return null;

            var recipeByLanguageOutput = _mapper.Map<RecipeByIdAndLanguageOutput>(
                _recipeRepository.RecipeByLanguage(_mapper.Map<RecipeByLanguageIn>(recipeByIdAndLanguageInput)));
        ...
        public IEnumerable<PropertiesListByTypeAndLanguageOutput> PropertiesListByTypeAndLanguage(
            PropertiesListByTypeAndLanguageInput propertiesListByTypeAndLanguageInput)
        {
            return
                _mapper.Map<IEnumerable<PropertiesListByTypeAndLanguageOutput>>(
                    _recipeRepository.PropertiesListByTypeAndLanguage(
                        _mapper.Map<PropertiesListByTypeAndLanguageIn>(propertiesListByTypeAndLanguageInput)));
        }

        public IEnumerable<PropertiesListByTypeLanguageAndRecipeOutput> PropertiesListByTypeLanguageAndRecipe(
            PropertiesListByTypeLanguageAndRecipeInput propertiesListByTypeLanguageAndRecipeInput)
        {
            return
                _mapper.Map<IEnumerable<PropertiesListByTypeLanguageAndRecipeOutput>>(
                    _recipeRepository.PropertiesListByTypeLanguageAndRecipe(
                        _mapper.Map<PropertiesListByTypeLanguageAndRecipeIn>(
                            propertiesListByTypeLanguageAndRecipeInput)));
        }
