 ...
 */
        ...
        (function($) {
            ...
            $.fn.countdown = function(options) {
                ...
                return this.each(function() {
                    ...
                    var intervalHandle = setInterval(function() {
                            // get the timerObject
                            const timerObject = makeTimer(upcomingDate, options);
                            // set the timerObject in options
                            options.timerObject = timerObject;
                            // create the HTML
                            const timerHtml = htmlParser(timerObject, options);
                            /**
            ...
            var scheduler = function(options) {
                 ...
                 */
                for (let day in options.schedule) {
                    // get the next upcoming date
                    const nextDate = nextDayByName(day);
                    // get the time foreach each future date and push it to upcomingDates
                    for (let time in options.schedule[day]) {
                        // create a time string to push onto the upcomingDates array
                        const timeString = nextDate + " " + options.schedule[day][time];
                ...
                const currentTimeParsed = Date.parse(currentTime) / 1000;
                 ...
                 */
                const shortTime = Math.min.apply(null, timeDifferencesParsed);
                for (let prop in timeDifferences) {
                 ...
                 */
                const scheduledDate = upcomingDates[shortTimeKey];
            ...
            var htmlParser = function(timerObject, options, format) {
                 ...
                 */
                var counter_years =
                    `<div class="years"><span class="count">${timerObject.years
                        }</span><span class="title">Years</span></div>`;
                var counter_days =
                    `<div class="days"><span class="count">${timerObject.days
                        }</span><span class="title">Days</span></div>`;
                var counter_hours =
                    `<div class="hours"><span class="count">${timerObject.hours
                        }</span><span class="title">Hours</span></div>`;
                var counter_minutes =
                    `<div class="minutes"><span class="count">${timerObject.minutes
                        }</span><span class="title">Minutes</span></div>`;
                var counter_seconds =
                    `<div class="seconds"><span class="count">${timerObject.seconds
                        }</span><span class="title">Seconds</span></div>`;
            ...
            var nextDayByName = function(scheduledDay) {
                 ...
                 */
                const nextDatebyDayofWeek = mm + " " + dd + ", " + yyyy;
